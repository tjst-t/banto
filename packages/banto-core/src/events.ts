...
export interface AuditVerdictEvent extends EventBase {
  ...
  /**
   * **既定で通した（フェイルオープン）印**（task-0287・ADR-0027）。
   *
   * 監査は合否の門ではなく補助の目にした——判定を出さずに報告した／判定を出さずに
   * 再試行の上限まで落ちた／監査セッションの spawn 自体が失敗した、の3つとも
   * `verdict: "pass"` として既定で通す。実装の正しさはマージ前ゲートの機械検証が
   * 担保する、という設計転換（PO 裁定 2026-08-20）を受けての承知の上のリスクである。
   *
   * **見張りの手段はこの印だけ。** 「監査が実質回っていないのに通った」件数を
   * あとから帳簿だけで数えられるようにするためのもので、これが無いとリスクを
   * 承知したことにならない。true のときだけ持つ（無いなら普通に判定を出した pass）。
   */
  byDefault?: boolean;
  /**
   * `byDefault` が true のときの理由（人が読む短文）。
   *
   * 「判定を出さずに報告」「判定を出さずに N 回落ちた」「監査セッションの spawn 失敗」の
   * どれだったかが、あとから帳簿だけで読めるようにする（I2：既定で通したことを黙らせない）。
   */
  defaultReason?: string;
  /**
   * **監査人が diff の外を読んだファイルと、その理由**（task-0287 契約改訂 a11・PO裁定 2026-08-20）。
   *
   * 監査手順は「まず diff を読み、diff だけで判断できるならそこで判定してよい。
   * 判断できないときだけ、その箇所に関係するファイルへ遡ってよい」（`buildAuditInstruction`）。
   * ファイルを読むこと自体は禁じない——禁じると diff だけでは判断できない変更で
   * 監査が嘘をつく。代わりに、遡ったならここへ理由を書かせる。
   *
   * **これは監査人の自己申告であって、機械が検出した事実ではない（I1）。**
   * pass/fail の判断材料には使わない——PO の狙いは2つ：(1) diff だけでどれくらい
   * 足りているかを数字で分かるようにする（「監査は diff で足りる」はいまは仮説）、
   * (2) 理由を書かせることで、監査人が漫然とリポジトリを読み歩くのを抑える。
   *
   * 任意なのは、この項目より前に書かれた帳簿を読めなくしないため。空配列は
   * 「diff の外を読まなかった」を表す（自己申告が無いのとは区別しない——両方とも
   * 「読んだと申告していない」で同じ扱い）。
   */

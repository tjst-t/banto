...
export interface LedgerEntry {
   ...
   *
   * **`"none"` を黙って省かない。** 隔離なしで回っていたことが、あとから台帳を見て
   * 分かる必要がある——2026-08-14 の事故は「配線はあるのに動いていない」を
   * 誰も知らないまま進んだ結果だった。
   */
  isolation?: "cgroup" | "none";
  /**
   * その職人の袋（cgroup）のディレクトリ（inc-0066 第2段）。
   *
   * 工房が落ちて起こし直したあとも、引き継いだ職人の袋を畳める（＝掃除で消さない）
   * ようにするために台帳へ載せる。隔離していないときは無い。
   */
  cgroupDir?: string;
}

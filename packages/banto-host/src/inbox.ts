...
export interface InboxEffect {
  ...
  /**
   * **押された札と回答**（`in-xxxxxxxx#approve`）を、この名前の引数として渡す（決定113）。
   *
   * 積む時点では札の id がまだ無い（`post` が採番する）ので、出どころは**押された時に**
   * ホストが埋める。Kobo の PO 承認のように「誰の・どの意思表示で通ったか」を
   * 帳簿へ残す口が要る——合言葉をやめた代わりに、監査可能性はこの記録が担う。
   *
   * 名前を積む側が決めるのは、ここに Kobo の都合を持ち込まないため（D5）。
   */
  originArg?: string;
}

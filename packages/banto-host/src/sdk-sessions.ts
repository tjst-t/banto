 ...
 * unit の上限のほうだった（**当時 3.00 GiB。2026-08-16 に 5.00 GiB へ上がっている**）。カーネルの Tasks state ダンプでは、その 3 GiB の
 ...
 *
 * 実測の制約は **上限本数 × 0.19 GiB ＋ node 本体 0.7 GiB ＜ unit の `MemoryMax`**。
 * **その `MemoryMax` は 2026-08-16 に 3 GiB から 5.00 GiB へ上がっている**
 * （`deploy/banto.service`、稼働機は drop-in `zz-memory.conf`）。**この注釈は 5.00 GiB を
 * 前提に計算している**——次に上限が動いたら、ここも計算し直すこと。
 *
 * 5.00 GiB での等号ぎりぎりは (5.00 − 0.7) ÷ 0.19 ＝ **22 本**。開いている会話 21 本を
 * 全部起こしても勘定は合うが、0.19 GiB/本は**平均であって天井ではない**（実測の幅は
 * 0.19〜0.23 GiB）ので、上限を計算上の限界に置くと平均が少しぶれただけで殺される。
 * 8 本なら 8×0.19 ＋ 0.7 ＝ 2.22 GiB で、**余裕は 2.78 GiB**——上振れしても、node 本体が
 * 膨らんでも当たらない。8 本を超えて同時に会話が動くことは実運用では無く
 * （開いている 21 本のうち動くのは数本）、超えても LRU で畳まれた側は次の発話で札から
 * 戻るだけなので、失うものは組み直しの一往復ぶんの待ちだけ。上げる判断が要るとしたら、
 * 根拠は空きメモリではなく `wakeStats()`（起こし直しの待ちが体感を悪くしているか）。
 */

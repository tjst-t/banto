...
export const PRESENTED_TOOL_NAMES: readonly NamespacedToolDefinition["name"][] = [
  ...
  "thread.settle",
  /**
   * **幹の判断で枝を終わらせる口**（PO実観測 2026-08-16・task-0234）。それまで幹には
   * `thread.steer` で「畳んでください」と頼むことしかできず、枝は知らせが来ないと
   * 動かないので頼んでも動くとは限らなかった——開いたままの枝が画面のレールに
   * たまる原因の1つだった。提示していないと、幹は自分の判断で枝を終えられない。
   */
  "thread.fold",
  /**

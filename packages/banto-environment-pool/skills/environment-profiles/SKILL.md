...
### サービスは常駐させる。走って終わるコマンドを `command` に書かない（imp-0028）

**検証プロファイルのサービスは、環境が畳まれるまで生きていなければならない。** 検証コマンドは
`env.run` が**別の使い捨てコンテナ**で走らせるもので、compose の `command` は
「その器を開けておく」ためだけにある。

```yaml
# docker/test.yaml
    command: ["sleep", "infinity"]        # ○ 畳まれるまで待つ
    # command: ["npm", "test"]            # ← × 走り終えて終了する＝環境が消える
```

`command` に走り終わるコマンドを書くと、**compose も provision も成功する**のに環境は
数十秒で消える。落ち方はこうなる（実機で踏んだ形）:

1. `env.provision` はコンテナが起きた直後に返る。`healthcheck` も「使えます」と答える
   ——その瞬間はまだ走っている最中なので `running`。嘘ではないが**一瞬の写し**でしかない
2. 数十秒後にコンテナが終了する
3. 以後の `env.run` は全部 exit 1

**いちばん紛らわしいのはマージ前ゲート**だ。1つの環境で受け入れ基準の本数ぶんコマンドを
走らせるので、**a1 だけ通って a2 以降が全滅する**——受け入れ基準の中身が悪いように見える。
しかも `env.verify`（立てて1コマンド走らせて畳む）は1本しか走らせないので**露見しない**：
プロファイルを作った時点の確認をすり抜ける。

機構の側も2段で断るようにしてある（それでも設計としては上が正しい）:

- **provision** — 起動直後に終わるサービスしか無ければ、その場で失敗する
  （「常駐している必要があります」と、直し方を言う）。ただし**捕まえられるのは
  「起動直後に終わる」ものだけ**——`npm test` のように数十秒走るものは、provision の
  時点ではまだ生きている
- **run / healthcheck** — 消えたあとは、**終了したサービス名と終了コード**を理由に載せる
  （`app は exit 0 で終了`）

短命な init コンテナと常駐サービスが混じっているのは構わない。判定は「**running が
1本も無ければ落とす**」なので、1本でも常駐していれば通る（終わっているものは注記に残る）。
...
2. `env.verify` を1回回す。立って・走って・畳むまでが通れば書けている
   ——ただし**1本しか走らせない**ので、常駐していないサービス定義はここを通ってしまう
   （上の「サービスは常駐させる」）。`env.provision` → 少し待つ → `env.run` を**2本**、
   まで見て初めて常駐を確かめたことになる

...
タスク定義に `environment: dev` と書いておくと、そのタスクが `in-review` に入った時点で
Kobo が環境を立て、**PO は札から開いて触れる**。判断が付いた瞬間に畳まれる。

- **ポート番号を Kobo に教える必要は無い**（決定60a）。Kobo は「人が触る」という意図だけを
  渡し、どのポートかは Environment Pool がこのファイルから決める
...
3. Kobo に載せているリポジトリなら、`environment:` を書いたタスクを1本 `in-review` まで
   運んで、**札に URL が載るか**を見る

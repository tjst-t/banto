...
export interface ViewSpec {
   ...
   * 設定・起点（下）はその**置き場が違うだけ**なので、印を1つ足して済ませる。
   *
   * `launcher`：**人が、AI の指しを待たずに直接開ける入口**（要件C3・PO指摘
   * 2026-08-25）。C14（`show`）は「AI が指す」を作ったが、C3は「人が開くことも
   * できる」も求めていた——それがまだ無かった。中身の機構は同じ（URIを台帳から
   * 引いて面に割り当てる）ので、`slot`の値を1つ増やすだけで足りる。
   */
  readonly slot?: 'canvas' | 'settings' | 'launcher';
  /**
   * この面**だけ**が使う実体。省くと `gui.entry`（モジュールの既定）を使う。
   *
   * **1モジュールが複数の`in-page`面を持てるようにするため**（PO指摘 2026-08-25：
   * fsが「ファイル」だけでなく「フォルダ」の面も持ちたい）。`gui.entry`は
   * 元々モジュール単位の1つだけだったが、それだと同じモジュールの別のURIプレフィクスに
   * 別のReactコンポーネントを割り当てられなかった。
   */
  readonly entry?: string;
  /**
   * `launcher`（上）が指す、実際に読める1つの URI。**省くと `uriPrefix` をそのまま使う**
   * ——`settings` のような、テンプレートを持たない固定1件の面ではそれで足りる。
   *
   * `uriPrefix` は**末尾が `/` の接頭辞**であることが多く（例：
   * `banto://fs/dir/`——このモジュールの下のどのディレクトリも開けるように）、
   * 接頭辞そのものは `{+path}` テンプレートの空文字マッチに対応しない
   * （実測 2026-08-25：MCP SDK の `ResourceTemplate` は末尾スラッシュのみの
   * URI を空文字にマッチさせない）。**根を指す実在の URI**（例：
   * `banto://fs/dir/.`）を、テンプレートを持つモジュール側が明示的に書く。
   */
  readonly launcherUri?: string;
}

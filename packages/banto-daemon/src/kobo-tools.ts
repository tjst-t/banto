...
export function createKoboTools(daemon: Daemon): NamespacedToolDefinition[] {
  ...
  const amend = defineNamespacedTool({
    ...
    description:
      ...
      "**緩める方向（スコープを広げる・基準を変える・条件を消す・レビューを緩める）は PO の判断**" +
      "なので、あなたには通せません——取次へ上げてください。**スコープの絞り込みはあなたが通せます**：" +
      "足したパスが**いまのスコープのどれかのパターンに覆われている**なら広げたとは読みません" +
      "（`tests/acceptance/**` → `tests/acceptance/foo.spec.ts` など）。逆に、いまのどのパターンにも" +
      "覆われないパスを1本でも足すなら PO の判断です。**glob を glob へ置き換えるとき**は、" +
      "いまのパターンが `<接頭辞>/**` 形の総取りで、改訂後のリテラル接頭辞がその中に収まるときだけ" +
      "覆われている扱いになります——それ以外の glob 同士（例 `src/**/*.ts` → `src/deep/**/*.ts`）は" +
      "**機械では読めないので PO へ回ります**。パスを消す（狭める）改訂はこれまでどおり通せます。" +
      "ただし**スコープを動かせば、狭めても監査は無効**になります。\n" +

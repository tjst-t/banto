...
// フィールドが無い項目は instance 既定を継承する。デモ用の初期値は持たない
// （決定・2026-09-03）——createRealProject（projects.ts）が新規Projectごとに追加する
...
// Project ↔ Module実装 の接続（§6.1）。デモ用の初期値は持たない（決定・2026-09-03）
...
// 同じパターン——`notifyMockStoreChange` で購読側に知らせる。デモ用の
// 初期値は持たない（決定・2026-09-03）

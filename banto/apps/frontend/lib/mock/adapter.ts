...
import { createRealChatModelAdapter } from "../backend/adapter";
// tool名の定数はlib/mock/tool-names.tsに切り出した（lib/backend/adapter.tsとの
// 循環importを避けるため）。ここまでの呼び出し元は変えずに済むよう再exportする。
import {
  HUMAN_TOOL_NAME,
...
export { HUMAN_TOOL_NAME, SHELL_RUN_COMMAND_TOOL_NAME, MODULE_RELAY_TOOL_NAME, APPROVAL_TOOL_NAMES };
...
export function createMockChatModelAdapter(thread: MockThread): ChatModelAdapter {
  // 実Projectのthread（lib/backend/projects.tsで作られたもの）は台本を持たない
  // ——実bantoホストに繋ぐ（決定・2026-09-03、モックの台本はそのまま残す）。
  if (thread.real) {
    return createRealChatModelAdapter(thread);
  }
  return {

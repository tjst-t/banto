...
| `BANTO_CHAPTER_MODEL` | （会話と同じモデル→番頭の標準） | 引き継ぎ資料と記憶の抽出に使うモデル。`provider/model-id`。**安いモデルを指定してよい**——会話とは別の呼び出しなので、本編のキャッシュに触らない。指定が無ければ会話のモデルを使うが、**会話が Claude Agent SDK のときそれは pi の台帳に無い**ので、さらに番頭の標準へ落ちる（inc-0068。畳めなかったときの断りに、実際に使われたモデルが出る） |

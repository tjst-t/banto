...
| `BANTO_CHAPTER_MODEL` | （画面の設定→既定 `claude-agent-sdk` の `haiku`） | 引き継ぎ資料の要約に使うモデル。**会話のモデルへは頼らない**（task-0151・inc-0068の教訓）。`provider/model-id`（従来どおり・pi のモデルとみなす）または `backend/provider/model-id`（`claude-agent-sdk` を明示するとき。例 `claude-agent-sdk/claude/haiku`）。**このタスクだけ優先順位が逆**——画面の設定「章の要約に使うモデル」より**この環境変数が勝つ**（運用に出ている互換のため）。指定が解決できないときは黙って別物へ落とさず、起動ログに警告を出し、章を畳めなかったときの断りに「指定・解決できなかった理由・実際に使ったもの」が出る |

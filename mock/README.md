...
### モバイル——上部バーを廃し、段を1つにする（2026-09-09、提案）

**発端**：ユーザー指摘「モバイルも同じように使いにくい」。**まだ決定ではない**
（採るなら `docs/specs/v4-frontend.md` に書き戻す）。

**何が使いにくかったか**（実測・390×844）：

| 症状 | 中身 |
|---|---|
| 段が2つ | 上部バー（50px）＋パネルのヘッダ（44px）＝94px が常時。**履歴・設定は両方の段にあった** |
| Project が読めない | 頭文字（b／自／記）だけの横スクロールの帯 |
| **Fork Thread に行けない** | モバイルの上部バーに Fork の口が無く、Command Palette で探すしかなかった（規則13の観点で問題——機能はあるのに画面から到達できない） |
| 的が小さい | アイコンが 28〜36px で画面最上部に9個密集 |
| デモのヒントが4行 | モックの都合が、狭い画面で会話そのものを押し出していた |

**直したこと**：

- **`MobileTopBar` を廃止**（ファイルごと削除）。ナビは**パネルのヘッダ左端の ≡ 1つ**に集約し、
  押すと**左から Drawer**（`MobileNavDrawer`、vaul——右スワイプで閉じられる）。
  段は1つ（48px）になり、**46px ぶんが会話に戻った**
- **Drawer の中身はデスクトップのサイドバーと同じ `NavPanel`**（新設・規則3）——
  Project 名も、その下の Base/Fork の目次も、モバイルで同じように読める。
  片方だけ直して食い違う状態を作らないよう、**目次の実装は1つ**にした
  （`project-rail.tsx` は「デスクトップの入れ物＋畳んだレール」だけを持つ）
- **受信箱だけはヘッダに残す**——判断待ちは「止まっている」ので、目次を開かなくても
  件数が見えるほうがよい。履歴は急がないので Drawer に譲った
- **Fork Thread へモバイルからも行ける**ようになった（Drawer の目次から。畳むボタンも
  タッチでは常時表示——hover が無いため）
- **`/settings` にも同じ ≡ を置いた**——モバイルはサイドバーが無いので、
  設定に入ったら Project に戻れない、という穴を塞ぐ
- **タップの的を大きく**——ヘッダのボタンはモバイルだけ 36px（デスクトップは 28px のまま）、
  ヘッダ自身も 48px
- **デモのヒントはモバイルで横1行のスクロール**に畳んだ（デスクトップは折り返しのまま）
- **採らなかった案**：下部タブバー（会話／受信箱／Project／設定）。banto のモバイルは
  下端を composer が占めるうえ、切り替える対象が「4つの固定タブ」ではなく
  **Project × Thread の2階層**なので、タブでは表現できない。目次を1枚持つほうが素直
- Playwright で実測（規則13・14、390×844・タッチ）：ヘッダが1段48pxになったこと／
  Drawer の中身（Project 3件・banto の目次3行）／Fork 行→`?fork=ui` かつ Drawer が閉じる／
  ← で Base に戻る／別 Project へ切替→ヘッダの題が「自宅サーバ」に変わる／
  ヘッダの受信箱→受信箱が開く／Drawer から検索・設定→開いた上で Drawer が閉じる／

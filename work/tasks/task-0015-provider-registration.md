...
type: task
kind: feature
title: 店（Provider）登録機構——接続情報・Tool・GUI・SKILLを1単位で登録する
status: draft
parent: epic-0002
depends: [task-0012]
refs: [adr-0010]
...
acceptance:
  - { id: a1, text: "店の登録単位が型として定義され、①接続情報 ②番頭へ提供するTool ③キャンバスへ提供するGUIカタログエントリ ④その店のSKILL を1つにまとめて登録できる（決定25・26）" }
  - { id: a2, text: "登録された店のToolが番頭のセッションへ、GUIエントリがキャンバスカタログへ、SKILLが番頭の手続き記憶へ、それぞれ自動で反映される（登録の口が1つで済む）" }
  - { id: a3, text: "UIは各GUIエントリについて、その店の接続情報を取得できる。コンポーネント側にエンドポイントを直書きしない" }
  - { id: a4, text: "同一kind・同一Tool名・同一SKILL名を複数の店が登録した場合、黙って上書きせずエラーになる（I2）" }
  - { id: a5, text: "番頭核のSKILLと店のSKILLが共存し、由来（どの店のものか）が判別できる。学習層による上書きはtask-0017の範囲だが、優先順位を差し込める形になっている（決定26）" }
...
決定26 により、登録単位には SKILL も含まれる（店が既定を出し、番頭の学習層が上書きする3層構成）。本タスクは「店の SKILL を受け取って番頭の手続き記憶へ載せる」ところまでを担い、学習層とオーバーライドの陳腐化検出は task-0017 とする。

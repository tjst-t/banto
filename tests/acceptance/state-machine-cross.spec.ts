...
import type { TaskPausedEvent, TaskResumedEvent, TaskFailedEvent, TaskSupersededEvent } from "@banto/core";
...
describe("[AC-S654396-2-3] Cross-cutting transitions: pause, resume, fail, supersede", () => {
  it("[AC-S654396-2-3] pause from implementing, verify task_paused event and paused status", async () => {
    ...
    try {
      ...
      // Check task_paused event in log
      ...
      // StateStore must reflect paused status with suspendedFrom
      const store = StateStore.replay(log);
      const task = store.getTask("task-0001");
      assert.ok(task !== undefined);
      assert.equal(task.status, "paused", "task status must be paused");
      assert.equal(task.suspendedFrom, "implementing", "suspendedFrom must be implementing");
  ...
  it("[AC-S654396-2-3] resume from paused restores to implementing", async () => {
    ...
    try {
      ...
      // StateStore reflects restored status
      const store = StateStore.replay(log);
      const task = store.getTask("task-0001");
      assert.ok(task !== undefined);
      assert.equal(task.status, "implementing", "task status must be restored to implementing");
      assert.equal(task.suspendedFrom, undefined, "suspendedFrom must be cleared after resume");
  ...
  it("[AC-S654396-2-3] fail() records task_failed event and sets status to failed", async () => {
    ...
    try {
      ...
      const result = StateMachine.fail(log, "task-0001", { reason: "回復不能エラー" }, "proj-cross");
      ...
      // StateStore reflects failed status
      const store = StateStore.replay(log);
      const task = store.getTask("task-0001");
      assert.ok(task !== undefined);
      assert.equal(task.status, "failed", "task status must be failed");
      assert.equal(task.failureReason, "回復不能エラー", "failureReason must be recorded");
  ...
  it("[AC-S654396-2-3] supersede() records task_superseded event and sets status to superseded", async () => {
    ...
    try {
      ...
      // Supersede task-0002 by task-0001
      const result = StateMachine.supersede(log, "task-0002", { by: "task-0001" }, "proj-cross");
      assert.equal(result.ok, true, "supersede() must succeed");

      // Check task_superseded event
      ...
      // StateStore reflects superseded status
      const store = StateStore.replay(log);
      const task2 = store.getTask("task-0002");
      assert.ok(task2 !== undefined);
      assert.equal(task2.status, "superseded", "task-0002 status must be superseded");
      assert.equal(task2.supersededBy, "task-0001", "supersededBy must be recorded");
  ...
  it("[AC-S654396-2-3] pause from non-pausable state (closed) is rejected and logged", async () => {

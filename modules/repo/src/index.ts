...
export function repoModule(root: string): DefinedModule {
  ...
  tools: (tool) => [
    ...
    tool({
      ...
      run: async (core, { branch, path: p }) => ({ path: await core.addWorktree(branch, p) }),
      summary: (v) => v.path,
    }),
    tool({
      name: 'has_worktree',
      description: 'Report whether a working tree exists at that path.',
      input: { path: z.string().describe('Working tree path, relative to the repo root') },
      output: { exists: z.boolean() },
      run: async (core, { path: p }) => ({ exists: await core.hasWorktree(p) }),
      summary: (v) => (v.exists ? 'ある' : '無い'),
    }),
    tool({
    ...
    tool({
      ...
      run: async (core, { ref }) => ({ commit: await core.headOf(ref) }),
      summary: (v) => v.commit,
    }),
    tool({
      name: 'is_ahead',
      description: 'Report whether a branch has commits the base does not.',
      input: {
        branch: z.string().describe('Branch to check'),
        base: z.string().optional().describe('Base branch (default "main")'),
      },
      output: { ahead: z.boolean() },
      run: async (core, { branch, base }) => ({ ahead: await core.isAhead(branch, base) }),

    ...
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      >
        <textarea
          ...
          className="w-full resize-none rounded-md border border-border bg-paper px-2.5 py-2 text-sm text-ink outline-none placeholder:text-ink-muted focus:border-border-strong"
        />
      ...
      {error && (
        <div className="flex items-start gap-2 border-b border-critical/30 bg-critical-soft px-3 py-2 text-xs text-critical">
      ...
      <ScrollArea className="min-h-0 flex-1">
        <div className="flex flex-col gap-2 p-3">
          {runs.length === 0 && (
            <p className="py-6 text-center text-sm text-ink-muted">依頼はまだありません。</p>
          )}
          {runs.map((run) => {
            ...
            return (
              <button
                ...
                className="rounded-md border border-border bg-surface px-3 py-2 text-left transition-colors hover:border-border-strong"
              >
                <p className="line-clamp-2 text-sm text-ink">{run.request}</p>
                <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-[11px] text-ink-muted">
                  ...
                  {run.failed ? (
                    <Badge tone="critical">失敗（人の判断待ち）</Badge>
                  ) : last === undefined ? (
                    ...
                    <Badge tone="critical">テスト失敗</Badge>
                  )}

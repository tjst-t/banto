...
| mode | banto が出す場所 |
|---|---|
| **`inline`** | 囲んでいる枠の中に埋め込む（会話のカード、設定画面の枠の中） |
...
- **切り替えは Thread 単位・セッション内で効く。** 他の Thread や新しい会話には
  影響しない——Configuration の既定値に戻る。**「いま自分がどのモードで
  会話しているか」を見失わないよう、composer のインジケータは常時表示する**
  （選んだ後、目立たなくなって忘れる、という事故を避ける）

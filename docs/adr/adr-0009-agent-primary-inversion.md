...
> status: **proposed**（探索ブランチ `explore/agent-primary` の working canon）。正式化は、PO が別途設計した「永続記憶システムを含む AI Agent」ドキュメントの統合相談を経てから。詳細は [docs/notes/agent-primary-inversion.md](../notes/agent-primary-inversion.md)。

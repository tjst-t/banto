...
**残りは決定98 で畳んだ**（task-0102・2026-08-13）：`NotSupported` の型 ／ Catalog を
ハーネスへの問い合わせに ／ `hostUsable`・`workerUsable` → `policy` 1つ ／
`LlmDefaults.workerTier` の撤去 ／ モデル操作 Tool 17→4。
**`ModelRef` の1文字列化だけは「しない」と決めた**（決定98c）。
...
**モデル操作の Tool は 19本 → 4本**（決定98f で完了）。設定変更は Tool ではなく GUI とファイルの担当にする
...
### 決定98: 決定94 の残りを畳む。**`ModelRef` は3フィールドのまま**

（PO裁定＋番頭裁定 2026-08-13・task-0102。決定94 の「概念を 7 → 5」を完了させる）

決定94 で畳めたのは束縛の表だけだった（`defaults.host` ＋ `picks` → `roles`）。残りは以下。

| | 畳んだもの | どうなったか |
|---|---|---|
| a | `NotSupported` を**値で返す** | `supports(ref) → true \| NotSupported`。「解決できなかった」と「この経路では原理的に回せない」は**直し方が違う**——前者はモデルを採用すれば直り、後者は経路を替えるしかない。`undefined` に潰していたので、画面に出せる理由が「使えません」だけだった |
| b | `hostUsable` ＋ `workerUsable` → **`policy` 1つ** | 用途は増える（要約器・抽出器はいまも別の口で選んでいる）。増えるたびに欄が増える形をやめ、1つの集合にした |
| c | **`ModelRef` は3フィールドのまま**（下記） | 1文字列化は**しない** |
| d | Catalog を**ハーネスへの問い合わせ**に | `CLAUDE_KNOWN_MODELS`（手書き3行）を直に読むのをやめ、`query().supportedModels()` を聞く |
| e | `LlmDefaults.workerTier` | 撤去。`defaults()` が常に `"standard"` を返す死んだ欄で、職人の既定等級は工房が持つ |
| f | モデル操作 Tool 17 → **4** | 番頭が持つのは `list` / `resolve` / `check_key` / `reload` ——**読みと診断だけ**。設定変更は GUI とファイルの担当（決定41c） |

#### c: `ModelRef` を1文字列にしない（PO裁定 2026-08-13）

`backend/provider/model` の3成分のまま行く。**`backend` は導出できない**という決定94 の
結論がそのまま効いており、1文字列にしても結局パースして3つに戻すだけで、
`worker.delegate` の `model` 引数と Kobo のタスク定義に破壊的変更が波及する。
**この項目は決定94 から落とす。**

#### d: 聞くと、手書きの表は既に古かった。**ただし聞いた一覧も全部ではない**

**実測（2026-08-13）**：`query().supportedModels()` は **LLM を1回も呼ばずに約1秒**で返り、
手書きの表（`opus` / `sonnet` / `haiku`）には無い `default` / `opus[1m]` /
`claude-fable-5[1m]` が並んだ。

**訂正（2026-08-13・実機で確認）**：この書き方は片側だけだった。
**聞いた一覧に素の `opus` は入っていない**が、`opus` は生きていて、しかも `opus[1m]` とは
**別のモデル**へ解決する（`claude-opus-5` と `claude-opus-5[1m]`——文脈長が違う）。

> **聞いた一覧は「勧める一覧」であって「使える名前の全部」ではない。**

実機の番頭は `opus` で動いていたので、聞いた一覧だけを選択肢に出すと
**いま効いている束縛が画面から消えた**（PO 報告）。**両方を足す**——聞いた側を先に、
足りないぶんを組み込みの別名から補う。

**待たない。** 聞いた結果は写しに置き、聞けるまでは組み込みの別名で答える
——起動を1秒遅らせない。**聞けなかったときも空にしない**（I2：答えを持っていないことと、
答えが「無い」ことは違う）。

これに伴い、設定画面の選択肢が**起動時に凍っていた**のも直した——区画は起動時に1回
組まれるので、`options` を配列で持つと、モデルを採用しても問い合わせが返っても
反映されない。

#### f: 在庫は減らさない

番頭に渡すのは4本だが、**HTTP 面（`coreTools`）は17本のまま**——設定画面の到達先なので、
絞ると画面が 404 になる（ADR-0019 決定82 と同じ理由）。

**移行**：`hostUsable`/`workerUsable` は読み込み時に `policy` へ移し、**古い欄は消す**。
実データ（87件）で確かめた——`host=true` 30件 → `policy⊃host` 30件、
`worker=true` 26件 → 26件、`contextWindow` 等の併記も落ちない。

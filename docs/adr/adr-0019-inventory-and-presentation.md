...
> （ADR-0017）の続きから採る。決定82・83・85・84-5 は実装済み。
> **決定84-1/2/3/4 も実装済み**（task-0100・2026-08-13）——保留の理由だった
> 「機械的な圧縮の害」は、**道具ごとに人が書き直す**ことで解いた（下記「決定84 の実装」）。
...
2. **説明文は短く。盛らない。** arXiv:2602.14878（856道具の実証）verbatim:
   ...
   **機械的にやってはいけない**（実測・条件C）。「説明文を第一文へ切り詰め、入れ子の説明を
   落とす」という一律処理は**不正な道具名を6件生んだ**（他の条件は0件）。短くすること自体は
   筋が良くても、やり方が違った——**道具ごとに人が書き直す**（task-0100 で実施）。
...
### 決定84 の実装（task-0100・2026-08-13）

**一律の機械処理をやめ、提示56本を1本ずつ人（職人）が書き直した。** 形は決定84-1〜4 のとおり
「目的1行＋入出力例（＋値を取るものは値の言語）」で、**手順は書かない**（段階的開示は
SKILL 側の仕事）。

書き直しの結果（提示分・`{name, description, parameters}` の JSON 文字数）:

| | 前 | 後 | |
|---|---|---|---|
| 定義の合計 | 30,943字 | 21,592字 | **-30.2%** |
| うち説明文 | 7,490字 | 8,293字 | +10.7%（例を足したので**増える**） |
| うちスキーマ | 20,401字 | 9,898字 | **-51.5%** |
| 入出力例のある道具 | 1 / 56 | **56 / 56** | 決定84-1 |
| `anyOf` / `oneOf` | 14（10本） | **0** | 決定84-3 |
| `patternProperties` | 3 | **0** | 同上 |

**減ったのは説明文ではなくスキーマ**、というのがこの書き直しの形をよく表している。
条件C の機械的圧縮は説明文を削って害を出したが、ここでやったのは逆で——**説明文には例を
足して増やし、スキーマから引数ごとの散文を落とした**（例が示していることを二重に書かない）。

平坦化の道具は `@banto/core` に置いた（`packages/banto-core/src/tool-schema.ts`）:

- `StringEnum(["rework", "reverify"])` — `Type.Union([Literal…])` が生む `anyOf` の入れ子を
  `{ type: "string", enum: [...] }` 1つにする
- `OpenObject()` — `Type.Record` が生む `patternProperties: { "^.*$": {} }`（正規表現をキーに
  した部分スキーマ）を `additionalProperties` 1つにする

**短くする過程で消してはいけないものが1つ見つかった。** `worker.delegate` の
`runtime` / `model` から**選べる名前の綴り**が落ちており、`claude-agent-worker.spec.ts`
（「番頭が綴りを当てにいかない」）が落ちた。**値の集合を示す説明は、例では代われない**
——例は1つの値しか見せられない。短い形で綴りだけ戻した。同じ理由で `inbox.post` の
`blocking`（名前だけだと真偽値に見えるが数）と `actions`（2〜4つ）も戻している。

#### 書き直しの前後を測った（n=114・対比較・McNemar）

**題材は選ばない。** `/var/lib/banto/threads/*.jsonl` から機械的に抽出した——
「PO の発話（10字以上）で、その直後に番頭が実際に道具を1本以上呼んだもの」だけを取る
（＝道具を使うのが正しい局面だと実績で分かっているもの）。311件が採れ、ログの並び順で
先頭から 114件を使った。

条件は**道具定義だけ**が違う（before = 書き直し前 / after = 書き直し後）。システム
プロンプト・題材・温度(0)・並びは同じで、**同じ題材を両条件に通す**。モデルは番頭が
実際に使うもの——ローカル vLLM の `deepseek-v4-flash-abliterated`。

| 指標 | before | after | discordant (b/c) | McNemar p |
|---|---|---|---|---|
| 道具を1本以上呼んだ | 112 / 114 | 112 / 114 | 1 / 1 | 1.0 |
| 呼んだ名前がすべて実在 | 111 / 114 | **112 / 114** | 1 / 2 | 1.0 |
| **存在しない道具名の呼び出し** | **1件**（`canvas__list_catalog`） | **0件** | — | — |

**悪化していない**（有意差なし・点推定はわずかに改善）。条件C で6件出た「存在しない
道具名」は 0件で、機械的圧縮との違いはここに出た。

**ただし天井に当たっている**——決定82・83（提示を56本に絞る）と 84-5（散文の一覧）が
既に効いていて、before の時点で 97.4% ある。**この題材ではこれ以上の差は測れない**ので、
「84-1/2/3 が更に効く」ことの証拠にはならない。言えるのは**害が無いこと**だけ。
引数の値の正しさ（決定84-4 の狙い）を測るには、別の物差し（呼んだ名前ではなく
**渡した値**を見る）が要る。

装置と生データ: `/var/lib/banto/artifacts/task-0100/`
（`extract.py` 題材抽出・`harness.py` 対比較・`result.final.jsonl` 全114件の結果）。
...
3. **引数の値が正しいか**（決定84-4 の狙い）。task-0100 の対比較は「呼んだ**名前**が
   実在するか」までしか見ておらず、`parameter value language mismatch` そのものは
   測れていない。**渡した値**を見る物差しが要る（識別子の欄に日本語が入っていないか、
   実在する envId / taskId を指しているか）。名前の物差しは既に天井（97.4%）に
   当たっているので、次に測るならここ

...
- **同じ10シナリオを回し直した**（[検証シナリオ](./memory-verification-scenarios.md) に保全）。判定は動く0／条件付き8／動かない2で見出しは動かないが、**穴の性質が「機構が無い」から「機構はあるが点火しない」へ移った**。解消＝訂正の派生伝播（3体指摘）／旧版の候補除外（3体）／origin 無視の最新性（2体）／職人の失敗の受け口 ほか。残存は spec-memory §20.8 に一覧。
...
**次コンテキストの入口**：ADR-0010（第2版）を PO が読む → accepted 化 ＋ spec-memory 本文の追随から。並行して §20.8 の残件（最優先＝検出の起点）。3決定は決着済みだが、**Policy/Loop の未定義7点と決定①の代償3点が残っている**（ADR-0010 §6・帰結に一覧）。これらは「他所の宿題」ではなく **banto の未着手設計**であり、厚みが要るのは Substrate でも Policy でもなく **driver（spec-memory Part III、§20・§21 が2節しかない）**。

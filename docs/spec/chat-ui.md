...
**章の要約に使うモデルは、畳む直前に引き直す**（PO報告 2026-08-20 → `b6e4fec6`）。設定画面の
「章の要約に使うモデル」（優先順位は環境変数 `BANTO_CHAPTER_MODEL` > 画面の設定 > 既定
`claude-agent-sdk`/`haiku`）は、会話の器を組んだときに一度だけ読んで焼き付けるのではなく、
**章を閉じる1回ごとに設定から解決し直す**。以前は器を組んだ時点の値を会話の生涯そのまま
使っていたため、走っている会話の途中で設定を変えても最後まで反映されなかった——PO からは
「設定を変えたのに要約が変わらない」としか見えない。畳む直前に引き直すことで、**走っている
会話でも、設定を変えれば次の1回から新しいモデルで要約される**。

指定が解決できないとき（登録に無いバックエンド・使えないモデル）は、**黙って既定へ落とさない**
（I2）。指定された名前・解決できなかった理由・実際に使ったものを添えて断る／警告する。ただし
同じ文言を畳むたびに出し続けるとログを荒らすので、直前に出したものと同じ内容なら重ねて出さない
（指定が変わった・直った、というときは必ず出る）。
...
——プロバイダによっては数百あり、並べた時点で選べなくなる（ADR-0011 決定138）。
...
思考レベル（Reasoning effort）を選び直すと、生きているセッションへその場で届く。以前は
中身のハーネス（`ClaudeAgentHarness`）だけが `setThinking` を持ち、間の皮（SDKセッションの
プール・`PooledSdkHarness`）を素通りしていたため、画面で思考レベルを変えても Claude Code
には一度も届いていなかった（PO報告 2026-08-20 → `b6e4fec6`）。

**思考レベルは中身ではなく、セッションの皮が覚える。** アイドル・本数の上限で会話のセッションが
畳まれ、次の発話で**組み直しても、指定は消えない**——中身だけが覚えていると、組み直した瞬間に
指定がサービス既定へ戻ってしまう。組み直すたびに、皮が保持している思考レベルを新しい中身の
起動パラメータへ渡す。

### 7.4.2 バックエンドを跨いだときの引き継ぎ

pi と Claude Code は別のセッションなので、モデル選択でバックエンドを跨ぐと、切替先は何も
知らない状態から始まる——PO から見れば会話の途中で番頭が記憶を失う（PO報告 2026-08-20 →
`b6e4fec6`、ADR-0020 決定93の呼び出し側）。

**バックエンドを跨ぐとき**（pi → Claude Code・Claude Code → pi の**両方向**）は、切替先へ
引き継ぎの種を渡す。種は生きているハーネスの `transcript()` からではなく、**会話の記録
（`Thread.transcript`）から**作る——器ごとの書き起こしは自分の分しか持たないため、
pi→Claude→pi と行き来した会話ではハーネス側の記録だと並びが狂う（古い分が新しい分より
後ろに来る）。会話の記録だけがバックエンドを跨いで順序どおりに並んでいる。

範囲は**いまの章のぶんだけ**（最後の章の区切りより後ろ）。それより前は章を畳んだときに
意図して落とした文脈で、ここで蘇らせると畳んだ意味が消える。載せるのは PO・番頭・知らせの
発言だけで、**思考とツールの入出力は落とす**（§7.3・決定28 と同じ理由——外部の文字列や
中身の判断過程が引き継ぎ資料を汚さないように）。

上限は **60,000 文字**。超えたら**古い側**（会話の古い方）から切り、削ったことを本文に
残す（「前略：書き起こしは全体で N 字あり…」）。話していない会話（章を畳んだ直後で、
区切りより後ろに何も無い）には種を作らない——空の種で始め直させるより、切替先を空のまま
始めさせるほうがまし。

**「切り替えても話の続きができる」一方で、思考とツールの呼び出し・結果は引き継がれない**
——切替先で必要なら道具で改めて引き直す。

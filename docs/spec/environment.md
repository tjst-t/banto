...
- `run` のタイムアウト規約（Environment Pool 側で一律か、プロファイルごとか）。§5.1 の上限と同じ性質なので、能力側の既定＋プロファイルで厳しくのみ可、が素直だが未裁定
- 隔離検証（自己更新パイプライン）用プロファイルの標準名
- **Environment Pool の HTTP 面の認証**（ADR-0010 決定27b／32d の帰結）。sops の復号鍵を持つモジュールなので、無認証だと credentials 経路が露出する。Worker Pool の報告経路も同じ面を使っており、モジュール HTTP 面全体の課題として詰める

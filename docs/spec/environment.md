...
- **知らせの届き先（2026-08-07 時点の実態・P3）**：畳み損ね・孤児は、Environment Pool が
  独立サービスになったため**サービスのログに出るだけ**で、番頭の会話には返らない
  （`env.list` には残るので画面からは見える）。番頭ホストに同居していた頃は会話へ流していた
  ——職人と同じく引きに行く形にするには、このモジュール側にイベントの口が要る（task-0066 の残り）

...
| 動詞 | 入力 | 出力 | 規約 |
|---|---|---|---|
| `provision` | config JSON, **envId**, taskId, **workdir**, **timeoutMs** | `{handle: {...}}` | handleは不透明JSON。呼び出し側は中身を解釈しない |
...
- ドライバの追加要件：管理下リソースには**envIdを含む命名**を適用すること（例：`banto-env-env-04479785fc`）
  - **taskId で名付けてはならない**（imp-0033・2026-08-15）。同じタスクで環境は複数立つ
    （Kobo がレビュー用に自動で立てた dev ／ 番頭が別に立てたもの ／ プロファイル違いの
    `env.verify`）。taskId で名付けると**それらが同じ資源を共有し、互いを壊す**——実測では
    あとから来た `compose up` が先の env のコンテナを作り直して公開ポートの実体を奪い、
    使い捨て側の `compose down` がレビュー環境のコンテナごと消した。どちらも PO が 502 を踏んだ
  - `envId` は Pool が provision の**前**に振り、台帳に載せるものと同じ値をドライバへ渡す
  - taskId は「何の検証か」のラベルとして台帳・handle に残る（名前には入らない）
...
- 名前空間（`banto-env-<envId>`）は**二重の守りの片方**であって、所有の根拠ではない
- **綴りが変わっても照合は落ちない。** 所有は記録（作ったときに書いた名前）で決まるので、
  旧名（`banto-env-task-*`）で立っている環境も記録に在るかぎり `list` に出て、台帳と
  突き合わされる。名前の綴りで濾していないことが、そのまま移行の安全でもある
...
- **`env.healthcheck` は、公開している環境については公開ポートを実際に叩く**（imp-0033）。
  ドライバの申告（docker なら `docker compose ps`）は「コンテナが動いている」ことしか言えず、
  **それが*この*環境のものかを言えない**——実測では実体を奪われた env が新しいコンテナを見て
  `ok: true` を返し、**「使えます」と答えた URL が 502 だった**。中継が上流として dial する
  相手（ホスト側の公開ポート）へ接続できるかを見て、繋がらなければ `ok: false`。
  **「叩けなかった」を「使えます」に丸めない**——接続を拒まれた（実体が無い）のか、時間内に
  判らなかったのかは `detail` で書き分ける（I1：分からないときは分からないと言う）。
  公開していない環境（`env.verify` の使い捨て等）は叩く先が無いので、ドライバの申告がそのまま答え
